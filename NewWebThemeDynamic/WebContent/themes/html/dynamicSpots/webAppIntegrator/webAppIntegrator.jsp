...
<c:set var="dirMD" value="${wp.selectionModel.selected.metadata}"/>
<c:set var="currentNavNodeID" scope="request" value="${wp.identification[wp.selectionModel.selected]}" />
...
<%
    ...
    if (processingSection.equals(WAI_NAV) || processingSection.equals(WAI_FOOTER)) {
        markupResult = markupResult.replaceAll("\\?uri", java.util.regex.Matcher.quoteReplacement(absolutePortalRootURL.toString())+"?uri");
    }
