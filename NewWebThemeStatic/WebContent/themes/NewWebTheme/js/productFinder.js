(function($) {
	...
	$.widget("canon.productfinder", {
	    ...
	    _createTypeahead: function(el) {
	    	var self = this;
	    	var outerDiv = $('<div></div>',{ class: 'col-xs-12 col-sm-6 col-sm-offset-3 text-center', id: 'typeahead'});
				...
				if ($this.find('span').hasClass('pf-open-off')) {
					...
					$.ajax({
						...
						$.each(level0, function(idx, option) {
							level0Dropdown.append($("<li class='col-xs-12 col-md-3 col-sm-4' data-depth='" + option.depth + "' data-name='" + option.name +  "' title='" + option.title + "'>" + decodeURIComponent(option.title) + "</li>"));
						});
