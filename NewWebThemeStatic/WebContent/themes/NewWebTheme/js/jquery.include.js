/*/////////////////////////////
Canon Plugins Included:

1.) jQuery 1.11.2
2.) Bootstrap 3.3.2
3.) Bootstrap - Date Picker plugin
4.) Bootstrap - Select plugin
5.) Bootstrap (3) Typeahead plugin
6.) Freewall library used for Quilt Ribbon
7.) pwstrength - 1.2.5 jQuery Password Strength plugin for Twitter Bootstrap
8.) Collapser - Plugin v2.0 - Heavily modified by Ryan Hollingsworth. 
9.) Cookie - added June15
10.) Froogaloop Vimeo API - Aug 11, 2015
11.) YouTube API - Aug 19, 2015
...
// Alex: Remove newline/tabs/spaces...
$(this).data( 'oCnt', s.e.html().replace(/^\s*|\s(?=\s)|\s*$/g, "") );

//console.log(s.e.html().replace(/^\s*|\s(?=\s)|\s*$/g, ""));
...
if (less.firstChild.nodeName == 'P'){
	console.log("P");
	lessBlock = less.firstChild;
}else{

	lessBlock = less; //less.childNodes;
}
...
//05-27-2015 Ryan Hollingsworth changed this var to preserve formatting
//06-12-2015 - 6-16-2015 updated, changing the location of truncation to accomodate for formatting tags
//var txt = $.trim(e.text());
var txt = $.trim(e[0].innerHTML);

//new function to ensure html tags are closed
...
var txtNoHtml = $.trim(e.text());
txtNoHtml = txtNoHtml.slice(0,parseInt(s.o.truncate));						

var txtWords  = txtNoHtml.split(" "); 
var wordRegEx  = "\\W+(?:\\w+\\W+){0,2}?"; //two, assuming a </b> and <p> tag? may need to increase
var b 		   = "";  //  "\\b"; b for whole words

var wordString = "";

for (var i =0; i < txtWords.length; i++){

if (i < txtWords.length-1){
wordString += txtWords[i] + wordRegEx;
}
else{
wordString += txtWords[i];
}
}

//var invalid = /[°"§%()\[\]{}=\\?´`'#<>|,;.:+_-]+/g;
//wordString = wordString.replace(invalid, "");

var pattern   = new RegExp( b + wordString + b );

var adjustedNum = txt.match(pattern);
var wordLength  = parseInt(s.o.truncate);
var wordIndex	= txtWords[txtWords.length-1].length;

if (adjustedNum !== null){ 
wordLength  = adjustedNum[0].length;
wordIndex	= txt.indexOf(adjustedNum[0]);
}

var truncateVar = wordIndex + wordLength;					
var showTextLength = s.o.showText.length+4; //"read more" + 4 is a space and ellipsis					
...
var a1 = txt.slice(0, truncateVar);
var a2 = txt.slice(truncateVar + showTextLength, txt.length);

txt = s.pad( checkHTML(a1), checkHTML(a2));
...
pad: function( a, b ){
	...
	//Ryan Hollingsworth edited this to put the ellipsis inside the tag with the content so it doesn't float to a new line
	
	//June 23, reverting back to original b/c we made futher adjustments, i think the original is cleaner and simpler.
	...
	$(lessBlock).append('<span class="coll-ellipsis">' + s.o.ellipsis + '</span>' + ( !$.isFunction( s.o.ctrlBtn ) ? s.ctrlHtml : '' ));
							
	return lessBlock.innerHTML ;
*/
...
if (!window['YT']) {var YT = {loading: 0,loaded: 0};}if (!window['YTConfig']) {var YTConfig = {'host': 'http://www.youtube.com'};}if (!YT.loading) {YT.loading = 1;(function(){var l = [];YT.ready = function(f) {if (YT.loaded) {f();} else {l.push(f);}};window.onYTReady = function() {YT.loaded = 1;for (var i = 0; i < l.length; i++) {try {l[i]();} catch (e) {}}};YT.setConfig = function(c) {for (var k in c) {if (c.hasOwnProperty(k)) {YTConfig[k] = c[k];}}};var a = document.createElement('script');a.type = 'text/javascript';a.id = 'www-widgetapi-script';a.src = 'https:' + '//s.ytimg.com/yts/jsbin/www-widgetapi-vfl3zSWzk/www-widgetapi.js';a.async = true;var b = document.getElementsByTagName('script')[0];b.parentNode.insertBefore(a, b);})();}